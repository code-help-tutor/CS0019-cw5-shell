...
open(FOO, "shell.c") || die "Did you delete shell.c?";
$lines = 0;
...
# Conditionals
    ...
    [ 'Test COND3',
      'grep -cv NotThere ../shell.c && echo Wanted',
      "$lines Wanted" ],

    [ 'Test COND4',
      'grep -c NotThere ../shell.c && echo Unwanted',
      '0' ],
...
# Pipelines
    ...
    [ 'Test PIPE5',
      '../shell -q cmd%%.sh; ps | grep sleep | grep -v grep | head -n 1 | wc -l',
      'Hello Bye 1',
      CMD_INIT => 'echo "echo Hello; sleep 2 & echo Bye" > cmd%%.sh'],
    ...
    [ 'Test PIPE13',
      ...
      'Hello 1',
      CMD_INIT => 'echo "echo Hello; sleep 0.4" > cmd%%.sh',
      CMD_CLEANUP => 'echo "sleep 0.2 ; ps T | grep sleep | grep -v grep | head -n 1 | wc -l" > cmd%%b.sh; ../shell -q cmd%%b.sh',
...
# Redirection
    ...
    [ 'Test REDIR5',
      'tr hb HB < f%%.txt | sort | ../shell -q cmd%%.sh',
...
# check for a ton of existing shell processes
$me = `id -un`;
chomp $me;
open(shell, "ps uxww | grep '^$me.*shell' | grep -v grep |");
$nshell = 0;
$nshell += 1 while (defined($_ = <shell>));
close shell;
if ($nshell > 5) {
    print STDERR "\n";
    print STDERR "${Red}**** Looks like $nshell ./shell processes are already running.\n";
    print STDERR "**** Do you want all those processes?\n";
    print STDERR "**** Run 'killall -9 shell' to kill them.${Off}\n\n";
}
...
sub run_shell ($;%) {
    ...
    unless (POSIX::setpgid($runshell_pid, $runshell_pid)) {
        if (!$!{EACCES}) {
            die("setpgid: $!\n");
        }
    }

    POSIX::tcsetpgrp(fileno(TTY), $runshell_pid) or die "tcsetpgrp: $!\n";
    ...
    eval {
        do {
            ...
            if (waitpid($runshell_pid, WNOHANG) > 0) {
                $answer->{"status"} = $?;
                die "!";
            }
            ...
            if (defined($size_limit) && $size_limit_file && @$size_limit_file) {
                my($len) = 0;
                $out = run_shell_pipe($out, fileno(OR), $size_limit);
    ...
    $answer->{"output"} = run_shell_pipe($out, fileno(OR), $size_limit);
    close(OR);
...
sub run (@) {
    ...
    my($info) = run_shell($cmd, "stdin" => $stdin, "stdout" => $outfile, "time_limit" => 10, "size_limit" => 1000, "dir" => "out", "nokill" => 1, "delay" => 0.05, "int_delay" => $opts{CMD_INT_DELAY});

    if ($opts{CMD_CLEANUP}) {
        if ($opts{CMD_CAREFUL_CLEANUP}) {
            my($infox) = run_shell("{ " . $opts{CMD_CLEANUP} . "; } >>$outfile 2>&1", "time_limit" => 5, "dir" => "out", "stdin" => "/dev/stdin", "stdout" => "/dev/stdout");
